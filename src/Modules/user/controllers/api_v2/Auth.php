...
class Auth extends REST_Controller
{
    ...
    public function login()
    {
        ...
        if (in_array($login['status'], ['failed', 'inactive'])) {
            $this->response([
                ...
                'response_message' => strip_tags($login['message'])
    ...
    public function get_jwt()
    {
        ...
        if ($jwt) 
            ...
            'response_message' => "Success.",
            "data" => [
                "token" => $this->ci_auth->getJWT()
            ]
        ]);

        $this->response([
            'response_code' => REST_Controller::HTTP_UNAUTHORIZED,
            'response_message' => "Session not exists. Please login first."
    ...
    public function register()
    {
        ...
        if ($register['status'] == 'failed') {
            $this->response([
                'response_code' => REST_Controller::HTTP_UNAUTHORIZED,
                'response_message' => strip_tags($register['message'])
            ], REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function reset_password()
    {
        ...
        if ($recovery['status'] == 'failed') {
            $this->response([
                'response_code' => REST_Controller::HTTP_UNAUTHORIZED,
                'response_message' => strip_tags($recovery['message']),
            ], REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function confirm_otp()
    {
        ...
        $this->response([
            "response_code" => $activated['status_code'] ?? REST_Controller::HTTP_OK,
            "response_message" => $activated['message'] ?? "Success",
            "data" => $activated
        ], $activated['status_code'] ?? REST_Controller::HTTP_OK);
    }
    ...
    public function reset_user_test()
    {
        ...
        if ($res)
            ...
            $this->response([
                "response_code" => self::HTTP_NOT_FOUND,
                "response_message" => 'Fail to delete, probably user not exist.'
            ]);
    }
}
